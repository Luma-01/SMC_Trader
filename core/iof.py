...
def is_iof_entry(htf_df: pd.DataFrame, ltf_df: pd.DataFrame, tick_size: Decimal) -> Tuple[bool, str]:
    ...
    if recent == 'BOS_up':
        ...
        bias = 'NONE'
    print(f"[BIAS] HTF 구조 기준 Bias = {bias} (최근 구조: {recent})")
    ...
    if bias in ['LONG', 'SHORT']:
        expected = 'long' if bias == 'LONG' else 'short'
        if expected == direction:
            print(f"[IOF] ✅ Bias와 진입 방향 일치 → Bias={bias}, Direction={direction}")
            #send_discord_debug(f"[IOF] ✅ Bias와 진입 방향 일치 → Bias={bias}, Direction={direction}", "aggregated")
        else:
            print(f"[IOF] ⚠️ Bias와 진입 방향 불일치 → Bias={bias}, Direction={direction}")
